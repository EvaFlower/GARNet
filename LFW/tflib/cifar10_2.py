...
def cifar_generator(filenames, batch_size, data_dir):
    ...
    images = np.concatenate([images[labels==0], images[labels==1]], axis=0)
    labels = np.concatenate([labels[labels==0], labels[labels==1]], axis=0)
...
def load_data(data_dir):
    ...
    # select two classes: 0, 1 (plane, car)
    train_images = np.concatenate([train_images[train_labels==0], train_images[train_labels==1]], axis=0)
    train_labels = np.concatenate([train_labels[train_labels==0], train_labels[train_labels==1]], axis=0)
    rng_state = np.random.get_state()
    np.random.shuffle(train_images)
    np.random.set_state(rng_state)
    np.random.shuffle(train_labels)

    test_images = np.concatenate([test_images[test_labels==0], test_images[test_labels==1]], axis=0)
    test_labels = np.concatenate([test_labels[test_labels==0], test_labels[test_labels==1]], axis=0)
    rng_state = np.random.get_state()
